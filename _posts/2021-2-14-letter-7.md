...
Assuming appropriate constants, I have modelled my satisfaction function as 
$\text{Satisfaction}(n) = n(5-n)$ [^5]
...
#### A homomorphic cryptography library in Haskell
   $NicheScore =  \frac{(75k-2.6k)}{75k} \times 2= 1.930\overline{6}$
...
Bad heuristics are great, they give you a direction, make you question that direct and best of all, keep changing taking into account newer circumstances. This heuristic is only the first iteration. Being reliant on bad heuristics is itself a bad heurisitic that may one day change for me [^7].
